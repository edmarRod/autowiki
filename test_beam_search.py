...
class T5Finetuner(pl.LightningModule):
    ...
    def test_epoch_end(self, outputs):
        ...
        self.log("avg_final_test_bleu", avg_bleu, prog_bar=True)
        self.log("avg_final_test_f1", avg_f1, prog_bar=True)
        self.log("avg_final_test_loss", avg_loss.detach(), prog_bar=True)
        print("avg_final_test_bleu:", avg_bleu)
        print("avg_final_test_f1:", avg_f1)
        wandb.log({'final_test_samples': self.wandb_table})
    ...
    def val_dataloader(self):
        dataset_val = MyDataset(text_pairs=x_val,
                                ...
                                source_max_length=self.source_max_length,
                                target_max_length=self.target_max_length)
        val_dataloader = DataLoader(dataset_val, batch_size=self.batch_size, shuffle=False, num_workers=0)
    ...
    def test_dataloader(self):
        ...
        test_dataloader = DataLoader(dataset_test, batch_size=self.batch_size, shuffle=False, num_workers=15)
...
if __name__ == '__main__':
    ...
    with open(path + 'final_test.json', 'r') as f:
        data = json.load(f)

    x_test = get_dataset(data)
    ...
    #                            shuffle=False,
    #                            num_workers=15)
    test_dataloader = DataLoader(dataset_test, batch_size=batch_size, shuffle=False, num_workers=15)
    ...
    #trainer.fit(model)
    model = T5Finetuner.load_from_checkpoint('/work/gabriel.santos/autowiki/checkpoints/t5_128_beam_search/epoch=9-step=11119.ckpt')
    trainer.test(model)
