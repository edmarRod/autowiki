...
class T5Finetuner(pl.LightningModule):
    ...
    def forward(self, source_token_ids, source_mask, target_token_ids=None,
        ...
        if self.training:
            ...
            generated_ids = self.model.generate(input_ids=source_token_ids,
                                                attention_mask=source_mask,
                                                max_length=self.target_max_length, do_sample=True, top_k=50, top_p=0.95, num_return_sequences=3)
    ...
    def test_dataloader(self):
        ...
        dataset_val = MyDataset(text_pairs=x_test,
                                tokenizer=self.tokenizer,
                                source_max_length=self.source_max_length,
                                target_max_length=self.target_max_length)
        val_dataloader = DataLoader(dataset_val, batch_size=self.batch_size,
...
if __name__ == '__main__':
    ...
    checkpoint_path = r"./checkpoints/t5_256/checkpoints.ckpt"
    ...
    #trainer.fit(model)
    model = T5Finetuner.load_from_checkpoint('/work/gabriel.santos/autowiki/checkpoints/t5_128_top_k/epoch=10-step=12230.ckpt')
    trainer.test(model)
